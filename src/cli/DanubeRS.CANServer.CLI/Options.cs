...
public interface IUploadOptions
{
    ...
    [Option("influxKey", Required = false, HelpText = "InfluxDB Key to upload with")]
    string InfluxKey {get;set;}
}
...
public class ParseOptions : IParseFromFilesystemOptions, IUploadOptions
{
    ...
    public required string InfluxKey { get; set; }
    public string? InputPath { get; set; }
}
...
[Verb("continuousDownloadAndParse", false, new[] { "cdp" }, HelpText = "Continuously downloads, parses, and stored logs; but only when the CANServer is not actively recieving")]
public class ContinuousDownloadAndParseOptions : IDownloadOptions, IParseOptions, IUploadOptions
{
    ...
    public string InfluxKey { get; set; }
}